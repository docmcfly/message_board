...
(function($) {
	...
	var inputosaurustext = {
		...
		parseInput : function(ev) {
			...
			} else if(!ev || ev.which === $.ui.keyCode.ENTER && !$('.ui-menu-item.ui-state-focus').length > 0 && !$('.ui-menu-item .ui-state-focus').length > 0  && !$('#ui-active-menuitem').length > 0){
				values.push(val);
				ev && ev.preventDefault();

			// prevent autoComplete menu click from causing a false 'blur'
			} else if(ev.type === 'blur' && !$('#ui-active-menuitem').length > 0 ){
				values.push(val);
			}
		...
		_inputBackspace : function(ev) {
			...
			if((!$(ev.currentTarget).val() || (('selectionStart' in ev.currentTarget) && ev.currentTarget.selectionStart === 0 && ev.currentTarget.selectionEnd === 0)) && lastTag.length > 0 ){
				ev.preventDefault();
		...
		_renderTags : function() {
			...
			$.each(this._chosenValues, function(k,v) {
				var className = self.options.classHook ? self.options.classHook(v.value) : null;
				var el = self._createTag(v.value, v.key, className);
